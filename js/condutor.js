...
CONDUTOR
======================================================
Interpreta o roteiro. Não desenha nada na tela.
======================================================
*/
...
const Condutor = {
    ...
    async texto(cena) {
        await Palco.mostrarTexto(this.substituir(cena.texto));
    ...
    async nome(cena) {
        ...
        MEMORIA.nome = await Palco.pedirNome(cena.pergunta);
        MEMORIA.salvar();
    },

    async escolha(cena) {
        await Palco.mostrarTexto(this.substituir(cena.pergunta));

        await Palco.esperar(cena.pausa ?? CONFIG.pausa.leitura);
        ...
        if (!resposta && Array.isArray(cena.seNegativo)) {
            for (const subCena of cena.seNegativo) {
                const continuar = await this.executarCena(subCena);
    ...
    async silencio(cena) {
        ...
        document.addEventListener("touchstart", contarToque);

        await Palco.esperar(cena.tempo ?? CONFIG.pausa.silencio);

        document.removeEventListener("click", contarToque);
        document.removeEventListener("touchstart", contarToque);

        if (MEMORIA.toquesNoSilencio > 0) {
            await Palco.mostrarTexto("Percebo que a espera não é fácil.");
            await Palco.esperar(CONFIG.pausa.media);

            await Palco.mostrarTexto("Reflita mais um instante.");
            await Palco.esperar(CONFIG.pausa.media);
        }
    },
    ...
    async fim(cena) {
        await Palco.mostrarTexto(this.substituir(cena.texto));
        MEMORIA.totalEncontros++;
        MEMORIA.encontroAtual = Math.min(MEMORIA.encontroAtual + 1, 3);
        MEMORIA.salvar();
    }
};
