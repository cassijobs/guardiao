...
const VERSOES={studio:"11.1",ar:"1.0.0",leitor:"1.0.0",app:"1.0.0",schema:1};
...
async function gerarLote(){
  ...
  ar.file("config.json",JSON.stringify({
    ...
    sistema:"O Escriba Studio 11.1 — Publicação de novo lote",
    versoes:VERSOES,
    geradoEm,
    lotes:configLotes
  },null,2));
...
async function publicarTudo(){
  ...
  ar.file("config.json",JSON.stringify({versao:Date.now(),sistema:"O Escriba Studio 11.1 — Biblioteca completa",versoes:VERSOES,geradoEm:geradoEmPublicacao,lotes:configLotes},null,2));
  ar.file("version.json",JSON.stringify({
    ecossistema:"Guardião",
    ...VERSOES,
...
async function canvasCartao(a){
  // Molde místico oficial CARD(3).
  // O símbolo de exemplo é substituído pelo PNG congelado correspondente ao artefato.
  const molde=await carregarImagemCartao("./assets/cartao-mistico.png");
  const c=document.createElement("canvas");
  c.width=molde.naturalWidth||908;
  c.height=molde.naturalHeight||1141;
  ...
  // Área circular do símbolo no CARD(3).
  const cx=c.width*.5,cy=c.height*.272,r=c.width*.235;
  x.save();
  x.beginPath();x.arc(cx,cy,r,0,Math.PI*2);x.clip();
  x.fillStyle="#b08a4f";x.fillRect(cx-r,cy-r,r*2,r*2);
  x.drawImage(im,cx-r,cy-r,r*2,r*2);
  x.restore();
  ...
  // Título e código, usando a mesma família tipográfica.
  x.font=`600 ${Math.round(c.width*.045)}px Georgia`;
  x.fillText("GUARDIÃO",cx,c.height*.055);
  x.font=`600 ${Math.round(c.width*.051)}px Georgia`;
  x.fillText(a.codigo,cx,c.height*.494);

  // Texto principal em evidência dentro do retângulo.
  x.font=`700 ${Math.round(c.width*.034)}px Georgia`;
  const frasePrincipal="SE ESTE SÍMBOLO CHEGOU ATÉ SUAS MÃOS, NOSSA CAMINHADA JÁ COMEÇOU.";
  textoCentralizado(x,frasePrincipal,cx,c.height*.575,c.width*.73,Math.round(c.height*.046),4);

  // Texto secundário, menor, logo abaixo.
  x.font=`500 ${Math.round(c.width*.021)}px Georgia`;
  const fraseSecundaria="Este símbolo foi criado exclusivamente para este artefato. Nenhum outro será exatamente igual.";
  textoCentralizado(x,fraseSecundaria,cx,c.height*.738,c.width*.72,Math.round(c.height*.029),3);

  // Rodapé dinâmico: muda de acordo com cada selo/artefato.
  const casa=tituloCasa(Simbolo.casa(a.codigo));
  const essencia=primeiraMaiuscula(Simbolo.significado(a.codigo));
  const tema=String(a.obs||"").trim()||essencia;
  const rodape=`${casa} - ${essencia} - ${primeiraMaiuscula(tema)}`.toUpperCase();
  x.font=`600 ${Math.round(c.width*.020)}px Georgia`;
  textoCentralizado(x,rodape,cx,c.height*.850,c.width*.74,Math.round(c.height*.027),2);
