...
async function imagemArtefato(a){
  // Também converte automaticamente símbolos antigos de 1000 px quando forem
  // visualizados, baixados ou incluídos num novo pacote AR.
  const original=a._imagem||await Simbolo.criar(a.codigo,a.estilo);
  a._imagem=await otimizarImagemSimbolo(original);
  return a._imagem;
}
...
async function publicarTudo(){
  ...
  ar.file("config.json",JSON.stringify({versao:7,sistema:"O Escriba Studio 10.4 — PNG 512 e Cartões Ajustados",geradoEm:new Date().toISOString(),lotes:configLotes},null,2));
