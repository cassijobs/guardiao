...
async function gerarLote(){
  ...
  const nome=l.nome;
  prog(2,"Preparando atualização incremental");
  ...
  ar.file("config.json",JSON.stringify({
    ...
    sistema:"O Escriba Studio 10.7 — Atualização incremental",
  ...
  download(b,`guardiao-atualizacao-${nome}.zip`);
  prog(100,"Atualização incremental criada");
  toast("Atualização do lote criada sem remover os lotes anteriores");
}
function config(){
...
async function publicarTudo(){
  ...
  ar.file("config.json",JSON.stringify({versao:Date.now(),sistema:"O Escriba Studio 10.7 — Publicação Incremental",geradoEm:new Date().toISOString(),lotes:configLotes},null,2));
