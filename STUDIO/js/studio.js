...
async function imagemArtefato(a){
  ...
  a._imagem=await Simbolo.criar(a.codigo,a.estilo);
  return a._imagem;
}
...
async function salvar(){
  const c=Simbolo.limpar($("codigo").value);
  if(dados.artefatos.some(a=>a.codigo===c))return toast("Este artefato já foi registrado");
  try{await registrarAtual()}catch(e){toast(e.message)}
}
function remover(id){if(!confirm("Remover este artefato da biblioteca?"))return;dados.artefatos=dados.artefatos.filter(a=>a.id!==id);organizar();render()}
function render(){
...
async function validarPacoteAR(ar,grupos){
  const obrigatorios=[
    "index.html","leitor.css","config.json",
    "js/app.js","js/config.js","js/identidade.js","js/reconhecimento.js","js/ritual.js","js/ui.js"
  ];
  for(const lote of grupos){
...
async function publicarTudo(){
  ...
  ar.file("config.json",JSON.stringify({versao:7,sistema:"O Escriba Studio 10.2 — Publicação Sincronizada",geradoEm:new Date().toISOString(),lotes:configLotes},null,2));
...
async function criarLoteCom25(){
  ...
  for(let i=0;i<TAMANHO_LOTE;i++){
    ...
    prog(2+(i+1)/TAMANHO_LOTE*70,`Gerando identidade ${i+1} de ${TAMANHO_LOTE}`);
...
async function exportarSimbolosDoLote(){
  ...
  for(let i=0;i<lote.artefatos.length;i++){
    ...
    prog(5+(i+1)/lote.artefatos.length*75,`Preparando símbolo ${i+1} de ${lote.artefatos.length}`);
  ...
  toast("Símbolos do lote exportados na ordem correta");
}
...
async function baixarPNG(){download(await Simbolo.blob(),`selo-${$("codigo").value}.png`)}

$("codigo").onchange=()=>{destino();revelar()};
$("novoCodigo").onclick=$("gerarCodigoGrande").onclick=()=>{$("codigo").value=novoCodigo();destino();revelar()};
$("revelar").onclick=revelar;$("salvar").onclick=salvar;$("baixarPNG").onclick=baixarPNG;
$("imprimirExplicacao").onclick=$("imprimirExplicacaoTopo").onclick=imprimirExplicacao;
$("limpar").onclick=()=>{$("obs").value="";$("codigo").value=novoCodigo();destino();revelar()};
$("busca").oninput=render;
document.body.onclick=e=>{
  const est=e.target.closest("[data-estilo]");
  if(est){document.querySelectorAll(".estilo").forEach(x=>x.classList.remove("ativo"));est.classList.add("ativo");$("estilo").value=est.dataset.estilo;revelar()}
...
async function reiniciarProjeto(){
  const aviso="Esta ação apagará a biblioteca, os lotes, os targets importados e o histórico local do Escriba. As jornadas e o site principal do Guardião não serão alterados. Continuar?";
  ...
  render();
  atualizarBoasVindas();
  prog(0,"Projeto reiniciado. Crie uma nova biblioteca ou restaure um backup.");
